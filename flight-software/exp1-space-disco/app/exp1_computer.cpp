...
void Exp1Computer::sensor_init() noexcept {
    if (!lp5810_rgb.init(&i2c, LP5810C_ADDR)) {
        on_sensor_failed();
        return;
    }
    if (!lp5810_uv_ir.init(&i2c, LP5810D_ADDR)) {
        on_sensor_failed();
        return;
    }
...
void Exp1Computer::try_spec_recovery() {
    ...
    case RecoveryStep::ASSERTING:
        ...
        HAL_GPIO_WritePin(SPEC_RESET_GPIO_Port, SPEC_RESET_Pin, GPIO_PIN_SET);
        recovery_ticks = 3U; // 3 x 40ms = 120ms >> 100ms boot time
...
void Exp1Computer::on_experiment_tick(uint16_t can_tick, uint32_t timestamp_us) {
    ...
    case 0: { // Verify LEDs off -> start dark measurement
        const bool rgb_off = lp5810_rgb.disable_all().has_value();
        const bool ir_off = lp5810_uv_ir.disable_all().has_value();
        start_tick = can_tick;
        start_timestamp_us = timestamp_us;
        const bool meas_ok = spec.start_measurement().has_value();
        result_valid = rgb_off && ir_off && meas_ok;
        break;
    }

    case 2: { // Read dark -> send dark -> enable RGB -> start RGB measurement
        result_valid = result_valid && spec.read_channels(&result).has_value();
        send_spectrum_pair(0);
        start_tick = can_tick;
        start_timestamp_us = timestamp_us;
        const bool led_ok = lp5810_rgb.set_channels(RGB_CHANNELS).has_value();
        const bool meas_ok = spec.start_measurement().has_value();
    ...
    case 4: { // Read RGB -> send RGB -> disable RGB, enable white -> start white measurement
        ...
        start_timestamp_us = timestamp_us;
        const bool rgb_off = lp5810_rgb.disable_all().has_value();
        const bool white_on = lp5810_uv_ir.set_channels(WHITE_CHANNEL).has_value();
        const bool meas_ok = spec.start_measurement().has_value();
    ...
    case 6: { // Read white -> send white -> switch to IR -> start IR measurement
        ...
        start_tick = can_tick;
        start_timestamp_us = timestamp_us;
        const bool ir_on = lp5810_uv_ir.set_channels(IR_CHANNEL).has_value();
        const bool meas_ok = spec.start_measurement().has_value();
    ...
    case 8: { // Read IR -> send IR -> switch to UV -> start UV measurement
        ...
        start_tick = can_tick;
        start_timestamp_us = timestamp_us;
        const bool uv_on = lp5810_uv_ir.set_channels(UV_CHANNEL).has_value();
        const bool meas_ok = spec.start_measurement().has_value();
