...
// LP5810 4-channel constant-current LED driver.
//
// I2C addressing: Addr = (chip_5bit << 2) | (reg >> 8)
// Pass the page-0 base address to init() - all registers used here are page 0 (0x000–0x0FF).
//
// LED control is purely via LED_EN (0x020): dot current and PWM are pre-armed in init().
// Call set_channels(mask) to illuminate; disable_all() to extinguish.
class LP5810 : public DeviceBase {
  public:
    // addr: page-0 I2C address (e.g. 0x14)
    // dc:   per-channel dot current 0x00–0xFF, scales 0–25.5 mA (max_current=0)
    [[nodiscard]] int init(CmsisI2CBus* bus, uint8_t addr, uint8_t dc = 0xFFU);

    // Write mask (bits 3:0) to LED_EN - enables those channels, disables the rest.
