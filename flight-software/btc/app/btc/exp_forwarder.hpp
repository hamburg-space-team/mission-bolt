...
///
/// @ingroup apps
[[nodiscard]] inline std::optional<PacketProtocol::NodeId> test_done_node(const uint8_t* buf, uint8_t len) noexcept {
    using namespace PacketProtocol;

    constexpr uint8_t test_packet_len =
        static_cast<uint8_t>(HEADER_SIZE) + static_cast<uint8_t>(sizeof(PayloadTest)) + static_cast<uint8_t>(CRC_SIZE);
