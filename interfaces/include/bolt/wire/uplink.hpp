...
namespace PacketProtocol {
    ...
    enum class CommandOpcode : uint8_t {
        ...
        STOP_EXPERIMENT WIRE(.desc = "stop the experiment sequence. Falls back to test sequence",
                             .label = "Stop Experiment", .danger = true) = 0x04,
        FULL_SYSTEM_TEST WIRE(.desc = "run the built-in self test", .label = "Full System Test", .danger = true) = 0x05,
    };

    /// Result reported back in PayloadCmdAck.status.
    /// @ingroup comms
    enum class CommandAckStatus : uint8_t {
        ACCEPTED WIRE(.desc = "known command received and executed (RESET_TICK/ACTIVATE_CAMERA: receipt only)") = 0x00,
