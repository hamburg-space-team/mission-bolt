...
import { r as reactExports, u as useMessages, s as saveState, j as jsxRuntimeExports, c as createRoot } from "./chunk-BMq7aTlC.js";
...
const _Accessor = class _Accessor extends Module {
  constructor(table) {
    super(table);
...
__publicField(_Accessor, "moduleName", "accessor");
//load defaults
__publicField(_Accessor, "accessors", defaultAccessors);
...
const _Ajax = class _Ajax extends Module {
  constructor(table) {
    super(table);
    this.config = {};
    this.url = "";
...
__publicField(_Ajax, "moduleName", "ajax");
//load defaults
__publicField(_Ajax, "defaultConfig", defaultConfig);
__publicField(_Ajax, "defaultURLGenerator", urlBuilder);
__publicField(_Ajax, "defaultLoaderPromise", defaultLoaderPromise);
__publicField(_Ajax, "contentTypeFormatters", defaultContentTypeFormatters);
let Ajax = _Ajax;
...
var defaultPasteParsers = {
  table: function(clipboard) {
    var data = [], headerFindSuccess = true, columns2 = this.table.columnManager.columns, columnMap = [], rows2 = [];
    ...
    if (data.length && !(data.length === 1 && data[0].length < 2)) {
      data[0].forEach(function(value) {
        var column = columns2.find(function(column2) {
  ...
  getColumns(traverse) {
    ...
    if (traverse) {
      this.columns.forEach((column) => {
        ...
        columns2 = columns2.concat(column.getColumns(true));
      });
    } else {
  ...
  updateData(updatedData) {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let attrname in updatedData) {
        let columns2 = this.table.columnManager.getColumnsByFieldRoot(attrname);
...
const _ColumnCalcs = class _ColumnCalcs extends Module {
  constructor(table) {
    super(table);
    this.topCalcs = [];
    this.botCalcs = [];
...
__publicField(_ColumnCalcs, "moduleName", "columnCalcs");
//load defaults
__publicField(_ColumnCalcs, "calculations", defaultCalculations);
let ColumnCalcs = _ColumnCalcs;
...
__publicField(DataTree, "moduleName", "dataTree");
...
const _Download = class _Download extends Module {
  constructor(table) {
    super(table);
...
__publicField(_Download, "moduleName", "download");
//load defaults
__publicField(_Download, "downloaders", defaultDownloaders);
let Download = _Download;
...
let Edit2 = _Edit;
class ExportRow {
  constructor(type, columns2, component, indent) {
    this.type = type;
...
const _Export = class _Export extends Module {
  ...
  generateExportList(config, style, range2, colVisProp) {
    var headers, body, columns2, colLookup;
    ...
    if (colLookup) {
      columns2 = colLookup.call(this.table);
      columns2 = columns2.filter((col) => this.columnVisCheck(col));
    }
    headers = this.config.columnHeaders !== false ? this.headersToExportRows(this.generateColumnGroupHeaders(columns2)) : [];
    if (columns2) {
      columns2 = columns2.map((col) => col.getComponent());
    }
    body = this.bodyToExportRows(this.rowLookup(range2), columns2);
  ...
  generateColumnGroupHeaders(columns2) {
    var output = [];
    if (!columns2) {
      columns2 = this.config.columnGroups !== false ? this.table.columnManager.columns : this.table.columnManager.columnsByIndex;
    }
    columns2.forEach((column) => {
    ...
    headers.forEach((header) => {
      ...
      header.forEach((col) => {
        if (col) {
          ...
          columns3.push(new ExportColumn(title, col.column.getComponent(), col.width, col.height, col.depth));
        } else {
          columns3.push(null);
        }
      });
  ...
  bodyToExportRows(rows2, columns2 = []) {
    var exportRows = [];
    if (columns2.length === 0) {
      this.table.columnManager.columnsByIndex.forEach((column) => {
        if (this.columnVisCheck(column)) {
          columns2.push(column.getComponent());
        }
      });
    }
    ...
    rows2.forEach((row, i) => {
      ...
      switch (row.type) {
        case "group":
          indent = row.level;
          exportCols.push(new ExportColumn(row.key, row.getComponent(), columns2.length, 1));
          break;
        case "calc":
        case "row":
...
const _Filter = class _Filter extends Module {
  constructor(table) {
    super(table);
    this.filterList = [];
    this.headerFilters = {};
...
__publicField(_Filter, "moduleName", "filter");
//load defaults
__publicField(_Filter, "filters", defaultFilters);
let Filter = _Filter;
...
class FrozenColumns extends Module {
  ...
  _calcSpace(columns2, index) {
    var width = 0;
    for (let i = 0; i < index; i++) {
      if (columns2[i].visible) {
        width += columns2[i].getWidth();
      }
    }
    return width;
  }
}
__publicField(FrozenColumns, "moduleName", "frozenColumns");
...
__publicField(FrozenRows, "moduleName", "frozenRows");
...
class GroupRows extends Module {
  ...
  virtualRenderFill() {
    ...
    if (layout === "fitDataFill" || layout === "fitDataStretch" || layout === "fitColumns") {
...
__publicField(GroupRows, "moduleName", "groupRows");
var defaultUndoers = {
...
const _History = class _History extends Module {
  constructor(table) {
    super(table);
    this.history = [];
    this.index = -1;
...
__publicField(HtmlTableImport, "moduleName", "htmlTableImport");
function csv(input2) {
...
const _Import = class _Import extends Module {
  constructor(table) {
    super(table);
  ...
  structureArrayToObject(parsedData) {
    var columns2 = this.transformHeader(parsedData.shift());
    var data = parsedData.map((values) => {
      var row = {};
      values = this.transformData(values);
      columns2.forEach((key, i) => {
        row[key] = values[i];
      });
      return row;
    });
  ...
  structureArrayToColumns(parsedData) {
    var data = [], firstRow = this.transformHeader(parsedData[0]), columns2 = this.table.getColumns();
    if (columns2[0] && firstRow[0]) {
      if (columns2[0].getDefinition().title === firstRow[0]) {
        parsedData.shift();
      }
    }
...
__publicField(_Import, "moduleName", "import");
//load defaults
__publicField(_Import, "importers", defaultImporters);
let Import = _Import;
...
const _MoveRows = class _MoveRows extends Module {
  constructor(table) {
    super(table);
    this.placeholderElement = this.createPlaceholderElement();
    this.hoverElement = false;
...
const _Page = class _Page extends Module {
  constructor(table) {
    super(table);
    this.mode = "local";
...
__publicField(_Page, "moduleName", "page");
//load defaults
__publicField(_Page, "pageCounters", defaultPageCounters);
let Page = _Page;
var defaultReaders = {
...
const _Persistence = class _Persistence extends Module {
  constructor(table) {
    super(table);
    this.mode = "";
    this.id = "";
...
__publicField(_Persistence, "moduleName", "persistence");
__publicField(_Persistence, "moduleInitOrder", -10);
//load defaults
__publicField(_Persistence, "readers", defaultReaders);
__publicField(_Persistence, "writers", defaultWriters);
...
__publicField(Popup2, "moduleName", "popup");
...
__publicField(Print, "moduleName", "print");
...
__publicField(ReactiveData, "moduleName", "reactiveData");
...
__publicField(ResponsiveLayout, "moduleName", "responsiveLayout");
__publicField(ResponsiveLayout, "moduleExtensions", extensions$2);
...
__publicField(SelectRow, "moduleName", "selectRow");
__publicField(SelectRow, "moduleExtensions", extensions$1);
...
class Range extends CoreFeature {
  ...
  getData() {
    var data = [], rows2 = this.getRows(), columns2 = this.getColumns();
    rows2.forEach((row) => {
  ...
  getCells(structured, component) {
    var cells = [], rows2 = this.getRows(), columns2 = this.getColumns();
    if (structured) {
      cells = rows2.map((row) => {
        var arr = [];
        row.getCells().forEach((cell) => {
          if (columns2.includes(cell.column)) {
...
var columnLookups = {
  range: function() {
    var columns2 = this.modules.selectRange.selectedColumns();
    if (this.columnManager.rowHeader) {
      columns2.unshift(this.columnManager.rowHeader);
    }
    return columns2;
  }
};
...
class SelectRange extends Module {
  ...
  findJumpCol(row, columns2, reverse, emptyStart, emptySide) {
    if (reverse) {
      columns2 = columns2.reverse();
    }
    return this.findJumpItem(emptyStart, emptySide, columns2, function(column) {
      return row.getData()[column.getField()];
    });
  }
  ...
  findJumpCellLeft(rowPos, colPos) {
    var row = this.getRowByRangePos(rowPos), columns2 = this.getTableColumns(), isStartingCellEmpty = this.isEmpty(row.getData()[columns2[colPos].getField()]), isLeftOfStartingCellEmpty = columns2[colPos - 1] ? this.isEmpty(row.getData()[columns2[colPos - 1].getField()]) : false, targetCols = this.rowHeader ? columns2.slice(1, colPos) : columns2.slice(0, colPos), jumpCol = this.findJumpCol(row, targetCols, true, isStartingCellEmpty, isLeftOfStartingCellEmpty);
    if (jumpCol) {
  ...
  findJumpCellRight(rowPos, colPos) {
    var row = this.getRowByRangePos(rowPos), columns2 = this.getTableColumns(), isStartingCellEmpty = this.isEmpty(row.getData()[columns2[colPos].getField()]), isRightOfStartingCellEmpty = columns2[colPos + 1] ? this.isEmpty(row.getData()[columns2[colPos + 1].getField()]) : false, jumpCol = this.findJumpCol(row, columns2.slice(colPos + 1, columns2.length), false, isStartingCellEmpty, isRightOfStartingCellEmpty);
    if (jumpCol) {
      return jumpCol.getPosition() - 1;
    }
    return colPos;
...
const _Sort = class _Sort extends Module {
  constructor(table) {
    super(table);
    this.sortList = [];
    this.changed = false;
...
__publicField(_Sort, "moduleName", "sort");
//load defaults
__publicField(_Sort, "sorters", defaultSorters);
let Sort = _Sort;
class GridCalculator {
  constructor(columns2, rows2) {
    this.columnCount = columns2;
    this.rowCount = rows2;
    this.columnString = [];
    this.columns = [];
    this.rows = [];
...
class SheetComponent {
  ...
  setColumns(columns2) {
    return this._sheet.setColumns(columns2);
  }
}
...
__publicField(Tooltip, "moduleName", "tooltip");
...
const _Validate = class _Validate extends Module {
  constructor(table) {
    super(table);
    this.invalidCells = [];
...
__publicField(_Validate, "moduleName", "validate");
//load defaults
__publicField(_Validate, "validators", defaultValidators);
...
class Renderer extends CoreFeature {
  ...
  reinitializeColumnWidths(columns2) {
  }
  renderRows() {
  }
  renderColumns() {
...
class ColumnManager extends CoreFeature {
  ...
  findColumn(subject) {
    ...
    if (typeof subject == "object") {
      if (subject instanceof Column) {
        ...
        let match = columns2.find((column) => {
  ...
  getComponents(structured) {
    var output = [], columns2 = structured ? this.columns : this.columnsByIndex;
    columns2.forEach((column) => {
      output.push(column.getComponent());
    });
    return output;
  ...
  _moveColumnInArray(columns2, from, to, after, updateRows) {
    var fromIndex = columns2.indexOf(from), toIndex, rows2 = [];
    if (fromIndex > -1) {
...
function fitData(columns2, forced) {
  if (forced) {
    this.table.columnManager.renderer.reinitializeColumnWidths(columns2);
  }
  if (this.table.options.responsiveLayout && this.table.modExists("responsiveLayout", true)) {
    this.table.modules.responsiveLayout.update();
  }
}
function fitDataGeneral(columns2, forced) {
  columns2.forEach(function(column) {
    column.reinitializeWidth();
  });
  if (this.table.options.responsiveLayout && this.table.modExists("responsiveLayout", true)) {
...
function fitColumns(columns2, forced) {
  ...
  function scaleColumns(columns3, freeSpace, colWidth, shrinkCols) {
    ...
    columns3.forEach(function(col, i) {
...
const NUM = /* @__PURE__ */ new Set(["seq", "tick", "timestamp_us", "fault_code", "error_code", "line", "depth", "opcode", "status", "reboot_count", "bit_errors", "bits_sent"]);
const TITLE = { seq: "#", timestamp_us: "t (µs)" };
...
function columns(rows2) {
  const fields = FIXED.slice();
  for (const row of rows2) for (const k of Object.keys(row)) {
    if (k !== "kind" && row[k] !== void 0 && !fields.includes(k)) fields.push(k);
  }
  return fields.map((f) => {
    const list2 = LIST.has(f);
    return {
      title: TITLE[f] ?? f,
      field: f,
      headerFilter: list2 ? "list" : "input",
      headerFilterParams: list2 ? { valuesLookup: true, clearable: true } : void 0,
      headerFilterLiveFilter: !list2,
...
function Packets() {
  ...
  const table = reactExports.useRef(null);
  ...
  reactExports.useEffect(() => {
    ...
    const t = new TabulatorFull(host.current, {
      ...
      columns: columns(rows2),
      columnDefaults: { headerSortTristate: true },
      layout: "fitDataStretch",
      height: "82vh",
      pagination: true,
      paginationSize: 100,
      paginationSizeSelector: [50, 100, 250, 1e3],
  ...
  const applyRange = () => {
    ...
    if (!t) return;
    const f = [];
    const add = (field, type, v) => v !== "" && f.push({ field, type, value: Number(v) });
    add("timestamp_us", ">=", t0);
    add("timestamp_us", "<=", t1);
    add("tick", ">=", k0);
    add("tick", "<=", k1);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bar", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "muted", children: "— filter any column header (node, fault_code, source, …)" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { flex: 1 } }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("input", { className: "num", placeholder: "t≥ µs", value: t0, onChange: (e) => setT0(e.target.value) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("input", { className: "num", placeholder: "t≤ µs", value: t1, onChange: (e) => setT1(e.target.value) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("input", { className: "num", placeholder: "tick≥", value: k0, onChange: (e) => setK0(e.target.value) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("input", { className: "num", placeholder: "tick≤", value: k1, onChange: (e) => setK1(e.target.value) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "secondary", onClick: applyRange, children: "Apply range" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "secondary", onClick: clearAll, children: "Clear" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: host })
  ] });
}
createRoot(document.getElementById("root")).render(/* @__PURE__ */ jsxRuntimeExports.jsx(Packets, {}));
