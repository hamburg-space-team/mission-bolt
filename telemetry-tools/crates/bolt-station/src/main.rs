...
//!
//! Owns the RXSM UART and re-serves the raw stream to any number of
//! clients, relaying their uplink back out. The same bytes are decoded
//! in-process for the HTTP API and the kiosk dashboard.
...
pub struct Config {
    /// RXSM byte source: serial device or tcp://host:port (dev mode)
    #[arg(long, default_value = "/dev/ttyRXSM")]
    port: String,

    /// Baud rate of the serial source
